...
|--------|-------------|-------|---------|
|--------|-------------|-------|---------|
| 404-count-in-logs | - | "Analyzes log files for 404 errors, counting occurrences per URL. Usage: ./scripts/404-count-in-logs /path/to/logs" | - |
...
| dbuild-force | Force rebuild and recreate Docker containers. Parameters: none. | - | - |
| dc | - | Usage: dc [docker compose arguments] | - |
...
| dkill | Stop Docker container. Parameters: [container_name] | - | - |
...
| dmaj | Update Docker containers: stop, pull git changes, rebuild and restart. Parameters: none. | - | - |
| dns-cloudflare-pour-les-eldinos | Configure Cloudflare DNS and block Proximus DNS for Xubuntu with NetworkManager. Parameters: none. | - | - |
...
| drestart | - | Stop Docker container. Usage: ./scripts/drestart [container_name_or_dir] | - |
...
| gcpAll | Run gcp on all Git repositories with uncommitted changes in current directory and subdirectories. Parameters: none. | - | - |
| gcp.backup | "Commit helper with auto-message generation via DeepSeek API, options: [-f|--force] [-p|--passtest] [-h|--help]" | - | - |
| git-add-private | Generate SSH key for a private Git repository and clone it. Parameters: <repository-url> | - | - |
...
| git-remove-secrets | Detect and remove secrets from Git history using Gitleaks and BFG. Parameters: none. | - | - |
| git-reset-clean | - | "Usage: ./scripts/git-reset-clean [-h] - Resets and cleans a git repo (hard reset, force clean), requires sudo." | - |
...
| warp | Create temporary user for warp-terminal session. Parameters: <user-number> | - | - |
| watch_connect_adb | Connect to Samsung Galaxy Watch via ADB with interactive prompts. Parameters: none. | - | - |
...
| ytdlp | Download YouTube videos with subtitles (French preferred, English fallback). Parameters: <url> <filename-without-extension> | - | - |
